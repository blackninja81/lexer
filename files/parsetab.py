...
_lr_productions = [
  ...
  ('expression -> expression PLUS term','expression',3,'p_expression_plus','passer.py',16),
  ('expression -> expression MINUS term','expression',3,'p_expression_minus','passer.py',20),
  ('expression -> term','expression',1,'p_expression_term','passer.py',24),
  ('term -> term TIMES factor','term',3,'p_term_times','passer.py',28),
  ('term -> term DIVIDE factor','term',3,'p_term_divide','passer.py',32),
  ('term -> factor','term',1,'p_term_factor','passer.py',36),
  ('factor -> NUMBER','factor',1,'p_factor_number','passer.py',40),
  ('factor -> LPAREN expression RPAREN','factor',3,'p_factor_expr','passer.py',44),
]
